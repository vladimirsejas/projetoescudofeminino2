...
df = pd.read_sql("""
SELECT tipo_cancer, SUM(valor_total) AS valor_total,
       AVG(valor_total) AS valor_medio, COUNT(*) AS internacoes
FROM internacoes
WHERE municipio = ?
GROUP BY tipo_cancer
""", conn, params=(MUNICIPIO,))
df = df.sort_values("valor_total", ascending=False)
df["ranking_custo"] = range(1, len(df) + 1)

print(f"\n=== CUSTOS HOSPITALARES ({MUNICIPIO}) ===\n")
print(df)
df.to_sql("custos_hospitalares", conn, if_exists="replace", index=False)
